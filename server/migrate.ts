...
async function migrate() {
    console.log('Spouštím databázovou migraci v Supabase...');
    ...
    try {
        ...
        // 3. Vytvoření prvního administrátora (pokud neexistuje)
        await client.query(`
            INSERT INTO users (email, password_hash)
            VALUES (
                'admin@coldmail.pro', 
                crypt('AdminSecretPassword123!', gen_salt('bf'))
            )
            ON CONFLICT (email) DO NOTHING;
        `);

        await client.query('COMMIT');
        console.log('✅ Migrace úspěšně dokončena! Všechny tabulky a indexy jsou připraveny.');
    } catch (error) {
...
// Umožňuje spuštění skriptu napřímo: npx tsx server/migrate.ts
